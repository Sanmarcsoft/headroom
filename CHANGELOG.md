...
### Fixed
- **mcp:** authenticate to a token-gated proxy. `headroom mcp serve` called `/v1/retrieve` and `/stats` with a bare HTTP client, so against a proxy started with `HEADROOM_PROXY_TOKEN` every remote MCP server got 401 on both: `headroom_retrieve` silently degraded to local-only (the proxy call is wrapped in a bare `except`) and `headroom_stats` returned local counters with the proxy summary quietly missing, neither of which reads as an auth failure. The shared client now carries `X-Headroom-Proxy-Token` when a token is configured via `HEADROOM_PROXY_TOKEN`, or via `HEADROOM_PROXY_TOKEN_FILE` for hosts that prefer to keep the secret out of the process environment (the env var wins; a blank, missing or unreadable file means "no token" and the server still starts and still compresses locally). A rejected token is now reported rather than swallowed: `headroom_stats` returns `proxy.status: unauthorized` with the proxy URL, and a retrieval miss that followed a 401/403 says the proxy rejected the token instead of looking like an ordinary cache miss. The token is never logged. Loopback callers are gated too (since the fork stopped exempting them), so same-host setups need the token as well unless the proxy sets `HEADROOM_PROXY_TOKEN_EXEMPT_LOOPBACK=1`.
