...
| `crewai` | CrewAI integration. This fork does not ship the extra: it pulled chromadb and json-repair into the lock for an integration nothing here runs. The module guards its own import, so install crewai alongside. | `pip install headroom-ai crewai` |
